...
namespace Whitehole
{
    public class Bmd
    {
        ...
        private void ReadMAT3()
        {
            ...
            ushort nummaterials = m_File.Reader.ReadUInt16();
            m_File.Stream.Position += 2;
            ...
            for (int i = 0; i < 30; i++) offsets[i] = m_File.Reader.ReadUInt32();
            ...
            for (int i = 0; i < nummaterials; i++)
            {
                ...
                // idk if that's right
                m_File.Stream.Position = sectionstart + offsets[2] + 4 + (i * 4) + 2;
                ushort nameoffset = m_File.Reader.ReadUInt16();
                m_File.Stream.Position = sectionstart + offsets[2] + nameoffset;
                mat.Name = m_File.ReadString();

                m_File.Stream.Position = sectionstart + offsets[1] + (i * 2);
                ushort matindex = m_File.Reader.ReadUInt16();

                m_File.Stream.Position = sectionstart + offsets[0] + (matindex * 0x14C);

                // giant chunk of crap here.
                // why everything has to be an index into some silly array, this
                // is beyond me
                mat.DrawFlag = m_File.Reader.ReadByte();
                byte cull_id = m_File.Reader.ReadByte();
                byte numchans_id = m_File.Reader.ReadByte();
                byte numtexgens_id = m_File.Reader.ReadByte();
                byte numtev_id = m_File.Reader.ReadByte();
                m_File.Stream.Position++; // index into matData6 -- 27
                byte zmode_id = m_File.Reader.ReadByte();
                m_File.Stream.Position++; // index into matData7 -- 28
                m_File.Stream.Position += 4; // color1 -- 5
                m_File.Stream.Position += 8; // chanControls -- 7?
                m_File.Stream.Position += 4; // color2 -- 8
                m_File.Stream.Position += 16; // lights -- 9
                ushort[] texgen_id = new ushort[8];
                for (int j = 0; j < 8; j++) texgen_id[j] = m_File.Reader.ReadUInt16();
                m_File.Stream.Position += 16; // texGenInfo2 -- 12
                m_File.Stream.Position += 20; // texMatrices -- 13?
                m_File.Stream.Position += 40; // dttMatrices -- 14?
                ushort[] texstage_id = new ushort[8];
                for (int j = 0; j < 8; j++) texstage_id[j] = m_File.Reader.ReadUInt16();
                ushort[] constcolor_id = new ushort[4];
                for (int j = 0; j < 4; j++) constcolor_id[j] = m_File.Reader.ReadUInt16();
                mat.ConstColorSel = new byte[16];
                for (int j = 0; j < 16; j++) mat.ConstColorSel[j] = m_File.Reader.ReadByte();
                mat.ConstAlphaSel = new byte[16];
                for (int j = 0; j < 16; j++) mat.ConstAlphaSel[j] = m_File.Reader.ReadByte();
                ushort[] tevorder_id = new ushort[16];
                for (int j = 0; j < 16; j++) tevorder_id[j] = m_File.Reader.ReadUInt16();
                ushort[] colors10_id = new ushort[4];
                for (int j = 0; j < 4; j++) colors10_id[j] = m_File.Reader.ReadUInt16();
                ushort[] tevstage_id = new ushort[16];
                for (int j = 0; j < 16; j++) tevstage_id[j] = m_File.Reader.ReadUInt16();
                ushort[] tevswap_id = new ushort[16];
                for (int j = 0; j < 16; j++) tevswap_id[j] = m_File.Reader.ReadUInt16();
                ushort[] tevswaptbl_id = new ushort[4];
                for (int j = 0; j < 4; j++) tevswaptbl_id[j] = m_File.Reader.ReadUInt16();
                m_File.Stream.Position += 24; // unknown6
                ushort fog_id = m_File.Reader.ReadUInt16();
                ushort alphacomp_id = m_File.Reader.ReadUInt16();
                ushort blendmode_id = m_File.Reader.ReadUInt16();

                m_File.Stream.Position = sectionstart + offsets[4] + (cull_id * 4);
                mat.CullMode = (byte)m_File.Reader.ReadUInt32();
                
                m_File.Stream.Position = sectionstart + offsets[6] + numchans_id;
                mat.NumChans = m_File.Reader.ReadByte();

                m_File.Stream.Position = sectionstart + offsets[10] + numtexgens_id;
                mat.NumTexgens = m_File.Reader.ReadByte();

                m_File.Stream.Position = sectionstart + offsets[19] + numtev_id;
                mat.NumTevStages = m_File.Reader.ReadByte();

                m_File.Stream.Position = sectionstart + offsets[26] + (zmode_id * 4);
                mat.ZMode.EnableZTest = m_File.Reader.ReadByte() != 0;
                mat.ZMode.Func = m_File.Reader.ReadByte();
                mat.ZMode.EnableZWrite = m_File.Reader.ReadByte() != 0;
                ...
                {
                    m_File.Stream.Position = sectionstart + offsets[11] + (texgen_id[j] * 4);

                    mat.TexGen[j].Type = m_File.Reader.ReadByte();
                    mat.TexGen[j].Src = m_File.Reader.ReadByte();
                    mat.TexGen[j].Matrix = m_File.Reader.ReadByte();
                }
                ...
                for (int j = 0; j < 8; j++)
                {
                    ...
                    m_File.Stream.Position = sectionstart + offsets[15] + (texstage_id[j] * 2);
                    mat.TexStages[j] = m_File.Reader.ReadUInt16();
                }
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    {
                        m_File.Stream.Position = sectionstart + offsets[18] + (constcolor_id[j] * 4);
                        mat.ConstColors[j].R = m_File.Reader.ReadByte();
                        mat.ConstColors[j].G = m_File.Reader.ReadByte();
                        mat.ConstColors[j].B = m_File.Reader.ReadByte();
                        mat.ConstColors[j].A = m_File.Reader.ReadByte();
                    }
                }
                ...
                {
                    m_File.Stream.Position = sectionstart + offsets[16] + (tevorder_id[j] * 4);

                    mat.TevOrder[j].TexcoordId = m_File.Reader.ReadByte();
                    mat.TevOrder[j].TexMap = m_File.Reader.ReadByte();
                    mat.TevOrder[j].ChanID = m_File.Reader.ReadByte();
                }
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    {
                        m_File.Stream.Position = sectionstart + offsets[17] + (colors10_id[j] * 8);
                        mat.ColorS10[j].R = m_File.Reader.ReadInt16();
                        mat.ColorS10[j].G = m_File.Reader.ReadInt16();
                        mat.ColorS10[j].B = m_File.Reader.ReadInt16();
                        mat.ColorS10[j].A = m_File.Reader.ReadInt16();
                    }
                }
                ...
                for (int j = 0; j < mat.NumTevStages; j++)
                {
                    m_File.Stream.Position = sectionstart + offsets[20] + (tevstage_id[j] * 20) + 1;

                    mat.TevStage[j].ColorIn = new byte[4];
                    for (int k = 0; k < 4; k++) mat.TevStage[j].ColorIn[k] = m_File.Reader.ReadByte();
                    mat.TevStage[j].ColorOp = m_File.Reader.ReadByte();
                    mat.TevStage[j].ColorBias = m_File.Reader.ReadByte();
                    mat.TevStage[j].ColorScale = m_File.Reader.ReadByte();
                    mat.TevStage[j].ColorClamp = m_File.Reader.ReadByte();
                    mat.TevStage[j].ColorRegID = m_File.Reader.ReadByte();

                    mat.TevStage[j].AlphaIn = new byte[4];
                    for (int k = 0; k < 4; k++) mat.TevStage[j].AlphaIn[k] = m_File.Reader.ReadByte();
                    mat.TevStage[j].AlphaOp = m_File.Reader.ReadByte();
                    mat.TevStage[j].AlphaBias = m_File.Reader.ReadByte();
                    mat.TevStage[j].AlphaScale = m_File.Reader.ReadByte();
                    mat.TevStage[j].AlphaClamp = m_File.Reader.ReadByte();
                    mat.TevStage[j].AlphaRegID = m_File.Reader.ReadByte();
                }
                ...
                for (int j = 0; j < mat.NumTevStages; j++)
                {
                    ...
                    {
                        m_File.Stream.Position = sectionstart + offsets[21] + (tevswap_id[j] * 4);

                        mat.TevSwapMode[j].RasSel = m_File.Reader.ReadByte();
                        mat.TevSwapMode[j].TexSel = m_File.Reader.ReadByte();
                    }
                }
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    m_File.Stream.Position = sectionstart + offsets[22] + (tevswaptbl_id[j] * 4);

                    mat.TevSwapTable[j].R = m_File.Reader.ReadByte();
                    mat.TevSwapTable[j].G = m_File.Reader.ReadByte();
                    mat.TevSwapTable[j].B = m_File.Reader.ReadByte();
                    mat.TevSwapTable[j].A = m_File.Reader.ReadByte();
                }

                m_File.Stream.Position = sectionstart + offsets[24] + (alphacomp_id * 8);
                mat.AlphaComp.Func0 = m_File.Reader.ReadByte();
                mat.AlphaComp.Ref0 = m_File.Reader.ReadByte();
                mat.AlphaComp.MergeFunc = m_File.Reader.ReadByte();
                mat.AlphaComp.Func1 = m_File.Reader.ReadByte();
                mat.AlphaComp.Ref1 = m_File.Reader.ReadByte();

                m_File.Stream.Position = sectionstart + offsets[25] + (blendmode_id * 4);
                mat.BlendMode.BlendMode = m_File.Reader.ReadByte();
                mat.BlendMode.SrcFactor = m_File.Reader.ReadByte();
                mat.BlendMode.DstFactor = m_File.Reader.ReadByte();
                mat.BlendMode.BlendOp = m_File.Reader.ReadByte();
        ...
        private void ReadTEX1()
        {
            ...
            ushort numtextures = m_File.Reader.ReadUInt16();
            m_File.Stream.Position += 2;

            Textures = new Texture[numtextures];

            uint entriesoffset = m_File.Reader.ReadUInt32();
            ...
            for (int i = 0; i < numtextures; i++)
            {
                ...
                m_File.Stream.Position = sectionstart + entriesoffset + (i * 32);

                tex.Format = m_File.Reader.ReadByte();
                m_File.Stream.Position++;
                tex.Width = m_File.Reader.ReadUInt16();
                tex.Height = m_File.Reader.ReadUInt16();

                tex.WrapS = m_File.Reader.ReadByte();
                tex.WrapT = m_File.Reader.ReadByte();

                m_File.Stream.Position++;

                tex.PaletteFormat = m_File.Reader.ReadByte();
                ushort palnumentries = m_File.Reader.ReadUInt16();
                uint paloffset = m_File.Reader.ReadUInt32();

                m_File.Stream.Position += 4;

                tex.MinFilter = m_File.Reader.ReadByte();
                tex.MagFilter = m_File.Reader.ReadByte();

                m_File.Stream.Position += 2;

                tex.MipmapCount = m_File.Reader.ReadByte();

                m_File.Stream.Position += 3;

                uint dataoffset = m_File.Reader.ReadUInt32();

                m_File.Stream.Position = sectionstart + dataoffset + 0x20 + (0x20 * i);
                                ...
                                for (int by = 0; by < height; by += 4)
                                {
                                    for (int bx = 0; bx < width; bx += 4)
                                    {
                                        for (int y = 0; y < 4; y++)
                                        {
                                            for (int x = 0; x < 4; x++)
                                            {
                                                ushort col = m_File.Reader.ReadUInt16();
                                ...
                                for (int by = 0; by < height; by += 8)
                                {
                                    for (int bx = 0; bx < width; bx += 8)
                                    {
                                        for (int sby = 0; sby < 8; sby += 4)
                                        {
                                            ...
                                            {
                                                ushort c1 = m_File.Reader.ReadUInt16();
                                                ushort c2 = m_File.Reader.ReadUInt16();
                                                uint block = m_File.Reader.ReadUInt32();
